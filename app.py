...
def load_model():
    ...
    # Subsample 20,000 rows for fast load time on free Streamlit Cloud containers
    ...
    # n_jobs=1 prevents container CPU locking
...
# Load data and model into session memory
flights_df, hotels_df, users_df, master_df = load_data()
model = load_model()

# Header Section
st.title("✈️ Voyage Analytics Platform")
st.markdown("An end-to-end travel analytics & flight price estimation dashboard.")
st.markdown("---")
...
with tab1:
    ...
    with col_left:
        ...
        fig_agency = px.bar(
            ...
            color='agency', 
            text_auto='.2s',
            labels={'agency': 'Agency', 'price': 'Revenue ($)'}
        )
...
# ==========================================
# TAB 2: ML FLIGHT PRICE PREDICTOR
# ==========================================
with tab2:
