...
with tab1:
    ...
    selected_agency = st.sidebar.multiselect("Select Agency", options=flights_df['agency'].unique(), default=flights_df['agency'].unique())
    selected_class = st.sidebar.multiselect("Select Flight Class", options=flights_df['flightType'].unique(), default=flights_df['flightType'].unique())
    
    filtered_df = master_df[(master_df['agency'].isin(selected_agency)) & (master_df['flightType'].isin(selected_class))]
    ...
    with col_left:
        ...
        fig_agency = px.bar(agency_rev, x='agency', y='price', color='agency', text_auto='.2s')
        st.plotly_chart(fig_agency, use_container_width=True)
        
    with col_right:
        st.subheader("Flight Class Distribution")
        class_counts = filtered_df['flightType'].value_counts().reset_index()
        fig_class = px.pie(class_counts, names='flightType', values='count', hole=0.4)
    ...
    fig_routes = px.bar(top_routes, x='Flight Count', y='Route', orientation='h', color='Flight Count', color_continuous_scale='Viridis')
...
with tab2:
    ...
    st.write("Fill in trip specifications below to get a machine learning price estimate.")
    ...
    with st.form("prediction_form"):
        ...
        with col_a:
            ...
            flight_time = st.number_input("Flight Time (hours)", min_value=0.5, max_value=12.0, value=1.8)
            
        with col_b:
            company = st.selectbox("Passenger Company", companies)
